...
def agent(image_bytes: bytes,
          agent_model: str = 'gemini-3-flash-preview',
          prompt: str = 'agent/prompt/few_shot.txt') -> str:
    """
    Analyzes an image using the Gemini model to extract inventory data.

    Reads a few-shot prompt from a file, initializes the Gemini client using
    environment variables, and sends the provided image bytes along with the
    prompt to the model.
